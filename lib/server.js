...
var baseURL = process.env.NODE_ENV == 'production'
  ? 'https://peaceful-earth-94201.herokuapp.com/' : 'http://localhost:5000/';

app.use(cookieParser());
var RedisStore = redis(session);
var store = new RedisStore( process.env.REDIS_URL ? { url: process.env.REDIS_URL } : {} );
app.use(session({
  secret: 'Shhhhh!',
  resave: false,
...
app.get('/partials/:template', function(request, response) {
